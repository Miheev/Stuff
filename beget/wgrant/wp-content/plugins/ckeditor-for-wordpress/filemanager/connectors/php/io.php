...
function SendUploadResults( $errorNumber, $fileUrl = '', $fileName = '', $customMsg = '' )
{
	...
	if ( 'QuickUpload' == $sCommand )
	{
		...
		if ($errorNumber && $errorNumber != 201) {
			...
			echo "<script type='text/javascript'>window.parent.CKEDITOR.tools.callFunction($funcNum, '$fileUrl', 'A file with the same name is already available. The uploaded file has been renamed.');</script>";
		}
		else {
	...
	else
	{
		...
		echo 'window.parent.OnUploadCompleted(' . $errorNumber . ',"' . $fileUrl . '","' . strtr( $fileName, $rpl ) . '", "' . strtr( $customMsg, $rpl ) . '") ;' ;
		echo '</script>' ;
	}
	exit ;
}
