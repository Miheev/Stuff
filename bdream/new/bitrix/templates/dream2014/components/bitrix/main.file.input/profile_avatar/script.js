...
BX.CFileInput.prototype.OnChange = function()
{
	...
	if(this.INPUT.files[0].size>1572864)
		{
			alert(BX.message("LIMIT_FILE_SIZE"));
			return false;
		}
	var uniqueID;
	do {
	...
	for(var i = 0; i < files.length; i++)
	{
		...
		if (files[i].fileID)
		{
			...
			var jcrop;
			 $.ajax({
				   type: "POST",
				   url: "/personal/profile/resize_img.php",
				   data: "IMG_ID="+files[i].fileID,
				   success: function(data){
					 $(".bigPhoto").html(data);
				     $(".miniPhoto div").html(data);
				 	
				     
				    var width = $('div.bigPhoto img').width(); 
				    var height =  $('div.bigPhoto img').height(); 
				    
				    var maxCropWidth = 200;
				    var maxCropHeight = 200;
				   
				    if(width<maxCropWidth)
				    	{
				    		var maxCropWidth = width;
				    	}
				
				    if(height<maxCropHeight)
				    	{
				    		var maxCropHeight = height;
				    	}
				    var aspectRatio = maxCropWidth/maxCropHeight;
					$('div.bigPhoto img').Jcrop({
				            setSelect:   [0, 0, maxCropWidth, maxCropHeight],
				            minSize: [maxCropWidth, maxCropHeight],
			...
			function showPreview(coords)
			{
			 ...
			 $('.cropImg').click(function() {
					...
					var imgSrc = $(".miniPhoto div img").attr("src");
					 $.ajax({
						   type: "POST",
						   url: "/personal/profile/crop_img.php",
						   data: "IMG_SRC="+imgSrc+"&W="+selection.w+"&H="+selection.h+"&X="+selection.x+"&Y="+selection.y,
						   dataType: "json",
						   success: function(data){
							    $(".bigPhoto").html("<img src='"+data.big+"' style='width:320px;'/>");
							    $(".miniPhoto div").html("<img src='"+data.min_big+"'/>");
							    if(data.post_file_src=="Y")
							    	{
							    		$(".bigPhoto").append("<input type='hidden' name='PHOTO' value='"+data.big+"' />");
							    	}
							    $("input.cropImg").hide();
						   }
						 });
