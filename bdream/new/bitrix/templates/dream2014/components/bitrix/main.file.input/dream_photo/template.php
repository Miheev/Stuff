...
<div class="file-input">
	...
	<div class="webform-field-upload">
		...
		<input type="file" name="<?=$arParams['INPUT_NAME_UNSAVED']?>[]" size="1"<?=$arParams['MULTIPLE'] == 'N' ? '' : ' multiple="multiple"';?> id="file_input_<?=$arParams['CONTROL_ID']?>" accept="image/jpeg,image/png,image/gif"/>
	</div>
</div>
<script type="text/javascript">
BX.message({
	MFI_CONFIRM: '<?=CUtil::JSEscape(GetMessage('MFI_CONFIRM'))?>',
	IMG_ADDED: '<?=GetMessage("IMG_ADDED");?>'
});
