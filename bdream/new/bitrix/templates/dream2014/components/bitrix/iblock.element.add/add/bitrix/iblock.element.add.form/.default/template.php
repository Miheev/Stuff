...
<?endif?>
<?
if($_GET["strIMessage"]!="" && $_SESSION["extion"]!="" && $_SESSION["ID"]!=""){
	LocalRedirect("/personal/add_dream.php?edit=Y&CODE=".$_SESSION["ID"]);
}else if($_GET["strIMessage"]!=""){
	global $USER;
	$arSelect = Array("ID");
	$arFilter = Array("IBLOCK_ID"=>2,"PROPERTY_USER"=>$USER->GetID());
	$res = CIBlockElement::GetList(Array("created_date"=>"desc"), $arFilter, false, Array("nTopCount"=>1), $arSelect);
	while($ob = $res->GetNextElement())
	{
	 $arFields = $ob->GetFields();
	 $code_elem=$arFields["ID"];
	}
	LocalRedirect("/personal/add_dream.php?edit=Y&CODE=".$code_elem);
}
$_SESSION["ID"]=$_GET["CODE"];
$_SESSION["extion"]=$_GET["edit"];
?>
<h1 id="pageTitleBlue">
<?if($_GET["edit"]=="Y"){echo "Редактирование мечты";}else{echo "Добавление мечты";}?>
...
$rsUsers = CUser::GetList(($by="ID"), ($order="desc"), $filter,array("SELECT"=>array("UF_VALUTA"))); // выбираем пользователей

while ($arUser = $rsUsers->Fetch()) 
{
   $valute=$arUser["UF_VALUTA"];
}
if($propertyID==39 && $valute==""){
	$none="display:none;";
}else $none="display:block;";?>
				<div class="line_<?=$propertyID?> inline" style="<?=$none?>">
					<div><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><span class="starrequired">*</span><?endif?></div>
					<div>
						<?
...
while ($arUser = $rsUsers->Fetch()) 
{
   $valute=$arUser["UF_VALUTA"];
					...
					if($valute==6) $valut_value="$";
					if($valute==4) $valut_value="Р";
					if($valute==7) $valut_value="€";
					if($valute==5) $valut_value="Ұ";
									?>
									<span class="curencyBtn activeCurency"><b><?=$valut_value?></b></span>
					<?}?>
					</div>
				</div>
				<div class="clear"></div>

						<input type="hidden" name="hidden_name_en" id="hidden_name_en" value="" />
						<input type="hidden" name="hidden_text_en" id="hidden_text_en" value="" />
			<?endforeach;?>
