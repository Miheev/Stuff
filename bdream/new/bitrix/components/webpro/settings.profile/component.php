...
if($_SERVER["REQUEST_METHOD"]=="POST" && ($_REQUEST["save"] <> '' || $_REQUEST["apply"] <> '') && check_bitrix_sessid())
{
	...
	if($strError == '')
		...
		{	
			...
			if (htmlspecialcharsbx($_REQUEST["settings_type"]) == 'save_psw')
			{
				
				if(htmlspecialcharsbx($_REQUEST["NEW_PASSWORD"]) <> '' && $arUser['EXTERNAL_AUTH_ID'] == '')
					{
						$arFields["PASSWORD"] = htmlspecialcharsbx($_REQUEST["NEW_PASSWORD"]);
						$arFields["CONFIRM_PASSWORD"] = htmlspecialcharsbx($_REQUEST["NEW_PASSWORD_CONFIRM"]);
					}
			...
			if ($_REQUEST["settings_type"] == 'save_mail')
			{
				
				if(((check_email($_REQUEST["email"])) ? "OK!" : "Error!") == 'OK!' && $arUser['EXTERNAL_AUTH_ID'] == '')
					{$arFields["EMAIL"] = $_REQUEST["email"];
				...
				else {$strError .= "Некорректный E-mail";}
			}
			
			if ($_REQUEST["settings_type"] == 'user_delete')
			{
			...
			if ($_REQUEST["settings_type"] == 'news_rss')
			{
				 $arFields = Array(
				...
				if($ID>0)
					...
					$strError .= "Error adding subscription: ".$subscr->LAST_ERROR."<br>";
			}
		}
