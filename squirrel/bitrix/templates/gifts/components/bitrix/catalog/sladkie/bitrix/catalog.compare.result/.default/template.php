...
<a name="compare_table"></a>
	...
	<?if($arResult["DIFFERENT"]):
		?><a href="<?=htmlspecialchars($APPLICATION->GetCurPageParam("DIFFERENT=N",array("DIFFERENT")))?>"><?=GetMessage("CATALOG_ALL_CHARACTERISTICS")?></a><?
	else:
		?><?=GetMessage("CATALOG_ALL_CHARACTERISTICS")?><?
	endif
	?>&nbsp;|&nbsp;<?
	if(!$arResult["DIFFERENT"]):
		?><a href="<?=htmlspecialchars($APPLICATION->GetCurPageParam("DIFFERENT=Y",array("DIFFERENT")))?>"><?=GetMessage("CATALOG_ONLY_DIFFERENT")?></a><?
	else:
	...
	<?if(count($arResult["DELETED_PROPERTIES"])>0):?>
		...
		<?foreach($arResult["DELETED_PROPERTIES"] as $arProperty):?>
			<a href="<?=htmlspecialchars($APPLICATION->GetCurPageParam("action=ADD_FEATURE&pr_code=".$arProperty["CODE"],array("pr_code","action")))?>"><?=$arProperty["NAME"]?></a>
		<?endforeach?>
		</p>
	<?endif?>
					...
					<?switch($code):
						case "NAME":
							...
							if($arElement["CAN_BUY"]):
								?><br /><a href="<?=$arElement["BUY_URL"]?>"><?=GetMessage("CATALOG_COMPARE_BUY"); ?></a><?
