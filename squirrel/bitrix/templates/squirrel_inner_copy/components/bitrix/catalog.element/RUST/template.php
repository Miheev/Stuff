...
<script>
    $(document).ready(function(){
        ...
        $('.bx_bigimages_imgcontainer img').click(function(e){
            e.stopPropagation();
            if ($(this).parent().find('img').length == 1) {
                $(this).parent().append($(this).clone());
                $(this).parent().append('<div class="close"></div>');
...
//            });
                ...
                $('.bx_bigimages_imgcontainer img.hover').remove();
                $('.bx_bigimages_imgcontainer .close').remove();
            }
        });
        $('body').click(function(){
            $('.bx_bigimages_imgcontainer img.hover').remove();
    ...
    <div class="item_info_section">
        ...
        if (!empty($arResult['DISPLAY_PROPERTIES']))
        {
            ...
            <dl>
                ...
                {
                    if ($arOneProp['CODE'] == 'to_stock') continue;
                    if ($arOneProp['CODE'] == 'remind') continue;
                    ?>
                ...
                if (intval($arResult["CATALOG_QUANTITY"]) == 0 &&
                    isset($arResult['DISPLAY_PROPERTIES']['to_stock']) &&
                    !empty($arResult['DISPLAY_PROPERTIES']['to_stock']['DISPLAY_VALUE']))
                { ?>
                    <dt><? echo $arResult['DISPLAY_PROPERTIES']['to_stock']['NAME']; ?></dt><?
                    echo '<dd>В течении '.$arResult['DISPLAY_PROPERTIES']['to_stock']['DISPLAY_VALUE'].'</dd>';
                }
                unset($arOneProp);
                ?>
                </dd></dl>
...
<div class="item_info_section">
    ...
    if ($canBuy)
    {
        ...
        $addBtnMessage = 'В корзину';
        $add_url= $arResult["ADD_URL"];
        $buyBtnClass = 'bx_big bx_bt_button bx_buy';
        $addBtnClass = 'bx_big bx_bt_button bx_cart';
    }
    else
    {
        $buyBtnMessage = ('' != $arParams['MESS_NOT_AVAILABLE'] ? $arParams['MESS_NOT_AVAILABLE'] : GetMessageJS('CT_BCE_CATALOG_NOT_AVAILABLE'));
        $addBtnMessage = 'Нет в наличии';
        $add_url= 'javascript:void(0);';
        $buyBtnClass = 'bx_big bx_bt_button_type_2 bx_buy';
        $addBtnClass = 'bx_big bx_bt_button_type_2 bx_cart bx_none';
    }
    if ('Y' == $arParams['USE_PRODUCT_QUANTITY'])
    {
        ?>
        <span class="item_section_name_gray"><? echo GetMessage('CATALOG_QUANTITY'); ?></span>
        <div class="item_buttons vam clearfix">
        ...
        <span class="item_buttons_counter_block">
            <a href="<?echo $add_url; ?>" class="<? echo $addBtnClass; ?>" rel="nofollow"><span></span><? echo $addBtnMessage; ?></a>
        </span>
            <!--        <span class="item_buttons_counter_block">-->
            <!--            <a href="--><?//echo $arResult["SUBSCRIBE_URL"]; ?><!--" class="" rel="nofollow"><span></span>Подписаться</a>-->
            <!--        </span>-->
...
<?php
//var_dump($arResult);
if ('Y' == $arParams['USE_PRODUCT_QUANTITY'] && intval($arResult["CATALOG_QUANTITY"]) == 0) : ?>
    <div class="stuff-reminder">
        <?php if (!isset($_GET['remind_stat'])) : ?>
            <form action="<?php echo $_SERVER["REAL_FILE_PATH"]; ?>" method="post">
...
<div class="main-column">
    ...
    if ('' != $arResult['PREVIEW_TEXT'])
    {
        ...
        {
            ?>
            <div class="item_info_section detail-text">
                <?
