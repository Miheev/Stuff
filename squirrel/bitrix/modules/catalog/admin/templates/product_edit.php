	...
	if (0 < $PRODUCT_ID)
	{
		...
		if ('Y' == $arMainCatalog['SUBSCRIPTION'])
		{
			$arProductSelect = array(
				...
				'PURCHASING_PRICE', 'PURCHASING_CURRENCY', 'BARCODE_MULTI', 'SUBSCRIBE_ORIG'
			);
		}
		else
		{
...
function OnChangePriceExist()
{
	...
	<tr id="tr_BASE_CURRENCY" style="display: <? echo ($bUseExtendedPrice ? 'none' : 'table-row'); ?>;">
		...
		$byCurrency = 'sort';
		$orderCurrency = 'asc';
		$db_curr = CCurrency::GetList($byCurrency, $orderCurrency);
		?>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table" id="catalog_properties_table">
	<tr id="CAT_BASE_QUANTITY2">
		...
		?>
			<input type="text" id="CAT_BASE_QUANTITY" name="CAT_BASE_QUANTITY" <?if ($bReadOnly || $bUseStoreControl) echo "disabled readonly" ?> value="<?echo htmlspecialcharsbx($str_CAT_BASE_QUANTITY) ?>" size="30">
		</td>
	</tr><?
		...
		<tr>
			...
			<td width="60%"><?
				...
				if(!empty($arAllMeasure)):?>
					<select style="max-width:220px" id="CAT_MEASURE" name="CAT_MEASURE" <?if ($bReadOnly) echo "disabled readonly"; ?>>
	...
	<?if(!empty($arAllMeasure)):?>
		<tr>
			...
			<td width="60%"><?
				...
				?>
				<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> id="CAT_MEASURE_RATIO" name="CAT_MEASURE_RATIO" value="<?echo htmlspecialcharsbx($str_CAT_MEASURE_RATIO) ?>" size="30">
	...
	<?endif;?>
		...
		<tr>
			...
			<td width="60%"><?
				...
				?>
				<select id="USE_STORE" name="USE_STORE" <? echo ($bReadOnly ? "disabled readonly" : 'onchange="onChangeCanBuyZero(this)"'); ?>>
		...
		<tr>
			...
			<td width="60%"><?
				...
				?>
				<select id="NEGATIVE_AMOUNT" name="NEGATIVE_AMOUNT" <?if ($bReadOnly) echo "disabled readonly"; ?><? if ($disableNegative) echo " disabled"; ?> data-oldvalue="<? echo $oldIndex; ?>">
