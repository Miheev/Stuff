...
foreach($arResult as $arItem):?>
	...
	<li class="HeaderNav"><a href="javascript:void(0)" class="showlist <?if ($arItem["SELECTED"]):?>root-item-selected<?else:?><?endif?>" id="<?=$arItem["ID"]?>"><?=$arItem["TEXT"]?></a>
				
				<ul class="HeaderUL open<?=$arItem["ID"]?>" <?if (!$childSelected && $bHasSelected){?>style="display:block;"<?}?>>
		<?else:?>
			<li><a href="<?=$arItem["LINK"]?>" class="showlist parent<?if ($arItem["SELECTED"]):?> item-selected<?endif?>" id="<?=$arItem["ID"]?>"><?=$arItem["TEXT"]?></a>
