...
<script>
    var it_sessid= BX.message('bitrix_sessid');
BX.ready( function(){
	if(BX.viewImageBind)
	{
		BX.viewImageBind(
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div class="blog-comment-content">
						...
						if(strlen($comment["urlToShow"])>0)
						{
							...
							<span class="blog-comment-show">
								<?if($arParams["AJAX_POST"] == "Y"):?>
									<a href="javascript:void(0)" onclick="return hideShowComment('<?=$comment["urlToShow"]."&"?>'+it_sessid, '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_SHOW")?>">
								<?else:?>
									<a href="javascript:void(0)" onclick="location.assign('<?=$comment["urlToShow"]."&"?>'+it_sessid); return false;" title="<?=GetMessage("BPC_MES_SHOW")?>">
								<?endif;?>
						...
						if(strlen($comment["urlToHide"])>0)
						{
							...
							<span class="blog-comment-show">
								<?if($arParams["AJAX_POST"] == "Y"):?>
									<a href="javascript:void(0)" onclick="return hideShowComment('<?=$comment["urlToHide"]."&"?>'+it_sessid+'&IBLOCK_ID=<?=$_REQUEST["IBLOCK_ID"]?>&ELEMENT_ID=<?=$_REQUEST["ELEMENT_ID"]?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_HIDE")?>">
								<?else:?>
									<a href="javascript:void(0)" onclick="location.assign('<?=$comment["urlToHide"]."&"?>'+it_sessid+'&IBLOCK_ID=<?=$_REQUEST["IBLOCK_ID"]?>&ELEMENT_ID=<?=$_REQUEST["ELEMENT_ID"]?>'); return false;" title="<?=GetMessage("BPC_MES_HIDE")?>">
								<?endif;?>
						...
						if(strlen($comment["urlToDelete"])>0)
						{
							...
							<span class="blog-comment-delete">
								<?if($arParams["AJAX_POST"] == "Y"):?>
									<a href="javascript:void(0)" onclick="if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) deleteComment('<?=$comment["urlToDelete"]."&"?>'+it_sessid+'&IBLOCK_ID=<?=$_REQUEST["IBLOCK_ID"]?>&ELEMENT_ID=<?=$_REQUEST["ELEMENT_ID"]?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_DELETE")?>">
								<?else:?>
									<a href="javascript:if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$comment["urlToDelete"]."&"?>'+it_sessid+'&IBLOCK_ID=<?=$_REQUEST["IBLOCK_ID"]?>&ELEMENT_ID=<?=$_REQUEST["ELEMENT_ID"]?>'" title="<?=GetMessage("BPC_MES_DELETE")?>">
								<?endif;?>
