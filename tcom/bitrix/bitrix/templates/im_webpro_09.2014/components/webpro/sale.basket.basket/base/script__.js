...
function recalcBasket(basketId) {
    form= $('#basket_form');
    //$('input[name="wposition"]').val($(window).scrollTop());
    //form.eq(0).submit();
    //if (2 > 1) return;
...
function recalcBasketRow(basketId) {
    el= $('#'+basketId);
    ...
    el.find('.price-full').text(Math.ceil(count_price)+currency);
    coeff= 0.18 - discount;
    nds= delta*coeff;
    nds_price= delta*(1+coeff);
    delta_price= delta*(1-discount);
    $('#allSum_FORMATED').text( Math.ceil(parseFloat($('#allSum_FORMATED').text().replace(/ /g,'').replace(',', '.')) + nds_price) + currency);
    $('#PRICE_WITHOUT_DISCOUNT').text( Math.ceil(parseFloat($('#PRICE_WITHOUT_DISCOUNT').text().replace(/ /g,'').replace(',', '.')) + delta) );
    $('#tax-price-0').text( Math.ceil(parseFloat($('#tax-price-0').text().replace(/ /g,'').replace(',', '.')) + nds) );
    $('#allVATSum_FORMATED').text( Math.ceil(parseFloat($('#allVATSum_FORMATED').text().replace(/ /g,'').replace(',', '.')) + nds_price) );
}
function recalcBasketAll() {location.reload();}
