...
function recalcBasketAjax(basketId)
{
	BX.showWait();
    ...
    //posturl = '/bitrix/components/webpro/sale.basket.basket/ajax_component.php';

    $.post(posturl, out, function(data, result){
        console.log(result);
        if (result == 'success') {
            if (typeof basketId != 'undefined') {
                recalcBasketRow(basketId);
                //location.reload();
                BX.closeWait();
            }
            else {
                console.log('It`s recalcBasketAll() -- check sale.basket.basket > script.js -- this message should not be displayed');
                recalcBasketAll();
            }
        } else
            console.log(data);
...
function recalcBasketRow(basketId, result) {
    el= $('#'+basketId);

    currency= ' Р';
    count= el.find('.count input[type="text"]').val();
    price= parseFloat(el.find('.price').eq(1).text().replace(/ /g,'').replace(',', '.'));
    cp_price= parseFloat(el.find('.price-full').text().replace(/ /g,'').replace(',', '.'));
    discount= parseFloat(el.find('.price-full').data('discount')) / 100;
    if (isNaN(discount))
        discount= 0.0;

    count_price= count*price*(1-discount);
    delta= count_price - cp_price;

    el.find('.price-full').text(Math.round(count_price)+currency);
    coeff= 0.0 - discount; //0.18
    nds= delta*0.18;
    nds_price= delta*(1+coeff);
    delta_price= delta*(1-discount);
    $('#allSum_FORMATED').text( Math.round(parseFloat($('#allSum_FORMATED').text().replace(/ /g,'').replace(',', '.')) + nds_price) + currency);
    $('#PRICE_WITHOUT_DISCOUNT').text( Math.round(parseFloat($('#PRICE_WITHOUT_DISCOUNT').text().replace(/ /g,'').replace(',', '.')) + delta) );
    $('#tax-price-0').text( Math.round(parseFloat($('#tax-price-0').text().replace(/ /g,'').replace(',', '.')) + nds) );
    $('#allVATSum_FORMATED').text( Math.round(parseFloat($('#allVATSum_FORMATED').text().replace(/ /g,'').replace(',', '.')) + nds_price) );
