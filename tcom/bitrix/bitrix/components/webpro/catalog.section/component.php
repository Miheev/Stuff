...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
    ...
    $true_filter= array_merge($arrFilter, $arFilter);
    $true_filter['INCLUDE_SUBSECTIONS']= 'N';
    ...
    if (empty($_GET['details'])) {
        ...
        $cats= array();
        $section_count= 0;
        $cats[]= $true_filter['SECTION_ID'];
        $rsParentSection = CIBlockSection::GetByID($true_filter['SECTION_ID']);
        if ($arParentSection = $rsParentSection->GetNext())
        {
            ...
            $rsSect = CIBlockSection::GetList(array('left_margin' => 'asc'),$arFilter, array('ID','NAME'));
            while ($arSect = $rsSect->GetNext())
            {
                $cats[]= $arSect['ID'];
//            echo $arSect['ID'].' '.$arSect['NAME'].'<br/>';
            }
            $section_count= count($cats);
        }
        $true_filter['SECTION_ID']= $cats;

        //EXECUTE
        $rsElements = CIBlockElement::GetList($arSort, $true_filter, false, array('nTopCount'=>500,'nPageSize'=>500,'bShowAll'=>'Y'), $arSelect);
        $els= array();
        ...
        usort ( $els, 'cmp_section');

        $prev_section= 0;
        $item_count= 0;
        $item_max= 4;
        $real_true_filter= array();
        foreach ($els as $iid => $el) {
            if ($prev_section != $el['IBLOCK_SECTION_ID']) {
                $item_count= 0;
                $prev_section= $el['IBLOCK_SECTION_ID'];
            }
            if ($item_count < $item_max) {
                $real_true_filter[]= $el['ID'];
                $item_count++;
            }
        }
        $true_filter['ID']= $real_true_filter;
        unset($true_filter['SECTION_ID']);
//    unset($true_filter['SECTION_GLOBAL_ACTIVE']); $arSort
    }

    $rsElements = CIBlockElement::GetList(array(), $true_filter, false, $arNavParams, $arSelect);
    ...
    if (count($cats) < 2 && empty($_GET['details']))
        LocalRedirect($arResult["SECTION_PAGE_URL"].'?details=1');

    if ( ceil($rsElements->NavRecordCount / $arNavigation["SIZEN"]) == $arNavigation["PAGEN"] )
        $arResult["last_page"] = 1;
...
//
//                    if ($prev_section != $arItem['~IBLOCK_SECTION_ID'] && $arIBlockSection = GetIBlockSection($arItem['~IBLOCK_SECTION_ID'])) {
...
//                $rs2 = CIBlockElement::GetList(array(), array('SECTION_ID' => $cat, 'INCLUDE_SUBSECTIONS' => 'Y'), false, array('nTopCount' => 4, 'nPageSize' => 4), $arResult['arSelect']);
//                while ($arItem = $rs2->GetNext()) {
//                    $arItem['ID'] = (int)$arItem['ID'];
//
//                    $arItem['ACTIVE_FROM'] = $arItem['DATE_ACTIVE_FROM'];
//                    $arItem['ACTIVE_TO'] = $arItem['DATE_ACTIVE_TO'];
